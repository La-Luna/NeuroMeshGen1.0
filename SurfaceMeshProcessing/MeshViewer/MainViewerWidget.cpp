...
void MainViewerWidget::OpenSwcGUI(const QString& fname) {
	if (fname.isEmpty() || !meshviewerwidget->LoadSwcFile(fname.toStdString())) {
		QString msg = "Cannot read swc file from file:\n '" + fname + "'";
		QMessageBox::critical(NULL, windowTitle(), msg);
	}
	else {
		loadmeshsuccess = true;
...
void MainViewerWidget::Open(void)
{
	QString fileName = QFileDialog::getOpenFileName(this,
		tr("Open mesh file"),
		tr(""),
		tr("Mesh Files (*.obj *.off *.ply *.stl *swc);;"
		"OFF Files (*.off);;"
		"OBJ Files (*.obj);;"
		"PLY Files (*.ply);;"
		"STL Files (*.stl);;"
...
void MainViewerWidget::OpenSWC(void) {
	QString filename = QFileDialog::getOpenFileName(this,
		tr("Open swc file"),
		tr(""),
		tr("SWC Files(*.swc)"
...
void MainViewerWidget::ShowSWCPoints(void) 
{
	meshviewerwidget->SetDrawMode(InteractiveViewerWidget::SWCPOINTS);
}

void MainViewerWidget::ShowSWCLines(void) {
	meshviewerwidget->SetDrawMode(InteractiveViewerWidget::SWCLINES);
}
void MainViewerWidget::ShowSWCCylinders(void) {
	meshviewerwidget->SetDrawMode(InteractiveViewerWidget::SWCCYLEN);
}
...
void MainViewerWidget::ShowGaussCurve() {
	meshviewerwidget->SetDrawMode(InteractiveViewerWidget::GAUSSCURV);
}
