...
SurfaceMeshProcessing::SurfaceMeshProcessing(QWidget *parent)
	: QMainWindow(parent)
{
	setWindowTitle(tr("NeuroMeshGen1.0"));
...
void SurfaceMeshProcessing::CreateActions(void)
{
	...
	actOpenSWC = new QAction(tr("&Open SWC file"),this);
	actOpenSWC->setStatusTip(tr("Open a swc file"));
	connect(actOpenSWC, SIGNAL(triggered()), viewer, SLOT(OpenSWC()));
	...
	actShowSWCPoints = new QAction(tr("SWCPoints"), this);
	actShowSWCPoints->setStatusTip(tr("Show SWC points"));
	actShowSWCPoints->setCheckable(true);
	connect(actShowSWCPoints, SIGNAL(triggered()), viewer, SLOT(ShowSWCPoints()));


	//ShowSWCCylinders
	actShowSWCLines = new QAction(tr("SWCLines"), this);
	actShowSWCLines->setStatusTip(tr("Show SWC lines"));
	actShowSWCLines->setCheckable(true);
	connect(actShowSWCLines, SIGNAL(triggered()), viewer, SLOT(ShowSWCLines()));

	actShowSWCCylinders = new QAction(tr("SWCCylinders"), this);
	actShowSWCCylinders->setStatusTip(tr("Show SWC cylinders"));
	actShowSWCCylinders->setCheckable(true);
	connect(actShowSWCCylinders, SIGNAL(triggered()), viewer, SLOT(ShowSWCCylinders()));
	...
	actShowGaussCurve = new QAction(tr("GaussCurve"), this);
	actShowGaussCurve->setIcon(QIcon(":/SurfaceMeshProcessing/Images/gaussCur.png"));
	actShowGaussCurve->setStatusTip(tr("show gauss curvature"));
	actShowGaussCurve->setCheckable(true);
	connect(actShowGaussCurve, SIGNAL(triggered()), viewer, SLOT(ShowGaussCurve()));
...
void SurfaceMeshProcessing::CreateMenus(void)
{
	...
	QMenu* menuGen = menuBar()->addMenu(tr("Gen"));
	QMenu *menuView = menuBar()->addMenu(tr("&View"));
	QMenu *menuRenderMode = menuView->addMenu(tr("Render Mode"));
	menuRenderMode->addAction(actShowSWCPoints);
	menuRenderMode->addAction(actShowSWCLines);
	menuRenderMode->addAction(actShowSWCCylinders);
	...
	menuRenderMode->addAction(actShowGaussCurve);
...
void SurfaceMeshProcessing::CreateToolBars(void)
{
	...
	tbView->addAction(actShowSWCPoints);
	tbView->addAction(actShowSWCLines);
	tbView->addAction(actShowSWCCylinders);
	...
	tbView->addAction(actShowGaussCurve);
	tbView->addSeparator()->setEnabled(false);
